...
export function buildIntrospectCheck({ jobStore, missionLoader, dependencies, llmLoader }) {
  return async function introspectCheck() {
    ...
    if (llmLoader && typeof llmLoader.introspect === 'function') {
      out.llm = llmLoader.introspect();
    }
    return out;
  };
}
