...
// LLM client is injected by boot path (loader-derived cascade-wrapped client)
// or by tests (mocks). MP-CONFIG-1 R5 — l9m-5.
...
export function createAPDrafter(config) {
  ...
  // MP-CONFIG-1 R5: boot path injects loader-derived client via `injectedLlm`.
  // Tests inject mocks the same way. Fall through to "unavailable" stub if neither.
  const llm = injectedLlm || {
    chat: async () => { throw new Error('Thalamus AP-Drafter: no LLM client wired; boot path must inject one (MP-CONFIG-1 R5)'); },
    isAvailable: () => false,
    getUsage: () => ({}),
  };
  ...
  async function draftAP(jobRecord) {
    ...
    //
    // MP-17 relay g7c-6 — tenant fields required on every APM. JobRecord is
    // the authoritative source; both tenant_urn and tenant_type were set at
    // intake (goal-intake = Cortex platform default; request-intake =
    // Receptor envelope forward). Fail-closed if either is absent — we must
    // not emit a tenant-less APM, as Nomos g7c-3's APM intake validator
    // rejects the envelope and Spine v1.1 APM schema expects both fields.
    if (!jobRecord.tenant_urn || !jobRecord.tenant_type) {
      log('thalamus_drafter_tenant_missing', {
        job_urn: jobRecord.job_urn,
        has_tenant_urn: Boolean(jobRecord.tenant_urn),
        has_tenant_type: Boolean(jobRecord.tenant_type),
      });
      degraded.push('tenant-context-missing-on-job-record');
      return { submitted: false, degraded };
    }
    const apmEnvelope = {
      type: 'APM',
      ...
      payload: {
        ...
        // MP-17 relay g7c-6 — tenant scoping for Nomos three-tier adjudication (g7c-3)
