...
export function createRequestIntake({ jobLifecycle, lifecycleAckEmitter }) {
  async function handleRequestEnvelope(envelope) {
    ...
    // MP-17 relay g7c-6 — tenant context required on every request. Receptor
    // sources tenant from Phi session introspection at ingress and forwards
    // the resolved tenant_urn + tenant_type in the envelope payload. Absent
    // tenant fields = malformed envelope (we cannot emit a tenant-less APM
    // per the relay guardrail). Fail structured at intake — visible to
    // Receptor — rather than later when Nomos rejects the APM.
    const tenant_urn = p.tenant_urn || null;
    const tenant_type = p.tenant_type || null;
    if (!tenant_urn || !tenant_type) {
      log('thalamus_request_intake_rejected', {
        reason: 'missing_tenant_context',
        payload_urn: p.payload_urn,
        note: 'Receptor must populate tenant_urn + tenant_type from Phi session introspection',
      });
      return { handled: false, error: 'missing_tenant_context' };
    }
    ...
    try {
      jobRecord = await jobLifecycle.createJob({
        ...
        priority,
        description,
        // MP-17 relay g7c-6 — forwarded from Receptor envelope
        tenant_urn,
        tenant_type,
      });
    } catch (err) {
